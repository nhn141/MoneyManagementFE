        ...
        val matchingKeywords = keywords.filter { keyword ->
            ...
            if (isMatch) {
                when {
                    exactMatch -> Log.d(
                        "CategoryIcon",
                        "  ✅ EXACT MATCH: '$keyword' exactly matches '$normalizedName'"
                    )

                    wordMatch -> Log.d(
                        "CategoryIcon",
                        "  ✅ WORD MATCH: keyword '$keyword' matches individual word in '$normalizedName'"
                    )

                    substringMatch -> Log.d(
                        "CategoryIcon",
                        "  ✅ SUBSTRING MATCH: '$normalizedName' contains substantial keyword '$keyword'"
                    )
                }
            } else {
                ...
                if (keyword.length < 3 && normalizedName.contains(keyword)) {
                    Log.d(
                        "CategoryIcon",
                        "  ❌ REJECTED: keyword '$keyword' too short for substring match in '$normalizedName'"
                    )
                }
            }
            isMatch
